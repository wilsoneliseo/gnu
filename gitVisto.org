...
** Pasos
1. Ver todos los branch, locales y remotos. (opcional)
#+BEGIN_EXAMPLE
$ git branch -a
#+END_EXAMPLE

2. Crear una nueva rama (y cambiarse hacia el) a partir del
...
3. Apilar los commits del la rama master, sobre la nueva rama creada
   (~nueva_historia~)
#+BEGIN_EXAMPLE
...
fc60f4f First prototype [Pepe Doval]
#+END_EXAMPLE
porque no esta el commit que queremos elimiar (bffd2e3)

4. pushear a origin esta nueva historia
#+BEGIN_EXAMPLE
$ git push origin nueva_historia
#+END_EXAMPLE
para ver las ramas locales y remotas puede hacer =$ git branch -a=

5. Ahora ha borrar la rama master en remoto (github), esto se hace
